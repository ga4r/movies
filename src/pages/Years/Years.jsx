...
function Years() {
  ...
  useEffect(() => {
    setFilteredMovies(movies.filter((movie) => movie.year === Number(year)));
  }, [year]);

  return (
    <div className="yearsPage">
      <div className="contentWrapper">
        <div className="yearsWrapper wrapper">
          <img
            src={filteredMovies[0].big_image}
            className="yearScreenBanner"
            alt=""
          />
          <BackButton marginBottom={"35px"} />
          <p className="yearDate"> Selected year: <span>{year}</span> </p>
